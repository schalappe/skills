...
description: Use when cleaning up commit history, squashing commits, cherry-picking changes across branches, finding bug-introducing commits, working on multiple branches simultaneously, or recovering lost work — any Git task beyond basic add/commit/push. Covers interactive rebase, cherry-pick, bisect, worktrees, and reflog recovery.
---
...
cd ~/projects/myapp
git fetch origin
# Cherry-pick the specific fix commit by hash
...
# Restore specific files from commit into working tree
...
1. **Always Use --force-with-lease**: Checks that the remote ref matches your local expectation before pushing — if a teammate pushed new commits you haven't fetched, the push is rejected instead of silently overwriting their work
...
For detailed rebase operations and conflict resolution:

- **`references/rebase-guide.md`** - Deep dive into interactive rebase, autosquash, split commits, and conflict resolution strategies
